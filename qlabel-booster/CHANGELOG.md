...
- v1.9.84 把目标脉冲错改成了 `outline-color` 颜色变化（视觉像静态加重红框，没有动态感）
- 现在改回经典的 `box-shadow` 扩散脉冲（红圈从中心向外扩散到 14px 后消失，循环波动）
- 与 `qlb-missing-highlight` 普通红框的区分：扩散幅度更大（14px vs 6px）、节奏更快（0.9s vs 1.4s）、颜色更深、outline 更粗
...
- **「定位未答题」会跳到首个真正的未答字段**（之前总是跳到某个 radio 评分组，即使排在 DOM 更前面的 textarea「评分原因」才是首个未答）
  - 根因：`scanMissingDetailed` 把 textarea 无脑追加到 fields 数组末尾，与 DOM 顺序无关
  - 修复：扫描结果按 `compareDocumentPosition` 排序，严格按 DOM 顺序返回
- **目标脉冲视觉差异更明显**：
  - 目标用 4px 深红 outline + 颜色脉冲 + 固定光晕（box-shadow）
  - 与 highlight（2px 浅红）视觉拉开层级，一眼能找到当前目标

---

## [1.9.83] - 2026-05-19

### 修复：面板「定位未答题」按钮和「提交拦截」视觉不一致
- 面板按钮之前只 setFocus 单个目标，**没给其它未答题加红框**；textarea 目标更是完全没红色反馈
- 现在统一走和"提交拦截"完全相同的视觉路径：
  - 所有未答题加红框（`qlb-missing-highlight`）
  - 目标加强脉冲（`qlb-missing-target`）
  - 安全视口对齐 + toast 提示
- 修复后行为：点提交 / 按 N 键 / 面板按钮 三个入口效果一模一样
...
### 改进：管理员浮动条收起态视觉
- 收起后显眼了：蓝色渐变胶囊 + 心跳脉冲光晕 + 朝向页面内的小箭头（左侧贴边箭头朝右、右侧贴边朝左），明确告诉你"点这里展开"
- hover 时停止脉冲并放大 6%（鼠标焦点反馈）
- 点击展开后会自动从边缘往里弹 40px，避免又立刻被吸附
...
### 新增：简版任务（无评分题）支持
- 识别新模板 `label-textonly`（多视频 + 评分原因 textarea，无打分 radio）
  - 之前因为没有 1-10 / 通过-不通过 信号，工具栏完全不出现
  - 现在会正常加载：视频悬浮窗、画中画、横向同步滚动、wheel-pan、草稿、定位未答题（textarea 维度）全部可用
- 工具栏识别简版任务后：
  - 头部 chip 显示「简版」
  - 进度栏只显示「评分原因 X/N」
  - 隐藏「一键全选 / 撤销」（无评分题不需要）
...
### 改进：管理员快速切换浮动条
- 屏蔽规则放宽：路径含 `/assignment/` 一律不显示
  - 之前 `/teamspace/{id}/assignment/{taskId}` 这种 teamspace 下的作业壳页会误显示
- SPA 路由切换时也会正确隐藏/显示（之前如果首次落地是不该显示的页面，监听就不启动了）

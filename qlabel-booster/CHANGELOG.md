...
### 修正 v1.9.62 的误改
- v1.9.62 错误地把质检模式按 `1`（通过）/ 鼠标点"通过"的"自动跳下一题"也接进了开关。
  这个体验属于**单题快捷键**，应该保持原样（按 1 永远跳下一题），不属于"维度打分自动跳转"的语义范围。
- 本版本回滚单题部分，恢复 v1.9.61 行为：按 `1` / 鼠标点"通过" → 永远跳下一题（与开关无关）。

### 改进
- 质检模式的**列顶"全部通过/不通过"批量按钮**接入"维度打分后自动跳转"开关：
  - 默认关闭：批量打分后保持当前位置
  - 开启：批量打分后自动定位到首个未答题（与标注模式的维度胶囊体验一致）
...
### 改进
- **改名**：UI 上"类别"统一改为"**维度**"，更准确反映标注页的术语
- **质检模式接入"维度打分后自动跳转"开关**（之前只标注模式有）：
  - 开关默认关闭：质检模式按 `1`（通过）/ 鼠标点"通过"后**保持当前焦点**（之前是无条件跳下一题）
  - 开关开启：恢复"打完通过自动跳到下一道未答"行为
  - 按 `2`（不通过）/ 鼠标点"不通过"后进 fix 模式（修正分输入）行为不变，与开关无关
- 工具栏开关 ID 名"维度打分后自动跳转"标注 + 质检共用，保持一致体验

...
### 改进：管理员快速切换浮动条
- **作业页面也不会出现了**：之前 `/workspace/assignment/progress/{n}/...` 任务详情壳页会误显示，现在已屏蔽
- **可拖动 + 位置记忆**：拖到任意位置自动保存，下次回来还在那里
- **贴边自动收起**：拖到屏幕左/右边缘 < 32px 时自动收成 ⚡ 小图标，点图标展开
- **文案精简**：项目 / 团队 / 个人（去掉冗长后缀）
- 当前所在区域仍高亮显示蓝色
...
### 新增：管理员后台快速切换
- 在 QLabel 后台管理界面（`/merchant/...` `/teamspace/...` `/workspace/...`）左上角显示一个浮动按钮组：
  - 🏢 项目管理中心
  - 👥 团队工作空间
  - 👤 个人工作台
- 当前所在区域会高亮显示蓝色
- **任务页（标注/质检 combinator）不显示**，作业人员看不到，只有管理员在后台才用得到
- SPA 路由切换时自动刷新可见性
...
### 帮助页文案精简 + 文档链接更新
- 工作流 / 批量 / 草稿三段说明改为一句话直白版
- 「评估标注标准细则」「质检问题共识」更新为最新文档链接
...
### 帮助页面按模板动态切换
- 美学专项（1-10 分制）现在有专属帮助页：
  - 打分快捷键：`1` ~ `9` → 1~9 分；`0` / `` ` `` / `~` → 10 分
  - 评分原因 textarea 用法 + Cmd/Ctrl+Enter 跳下一题
  - 导航：⬇/Tab 在评分 ↔ 评分原因 ↔ 维度评分之间按 DOM 顺序流转
  - 工作流 / 批量提示也按新模板重写
- 旧 4 档模板的帮助页保持原样，零回归
- 头部 badge 区分「标注模式 · 美学专项 1-10」/「标注模式 · 4 档评分」/「质检模式」
...
### 修复 + 文案
- **进度文案更直观**：`0/36` → `评分 0/36 · 评分原因 0/N`
- **修复"评分原因"数量不对**：之前抓的是页面上**所有 textarea**（含任务说明等不相关字段），导致显示 13 个等异常数字
  - 现在严格限定到评分列容器（`.cr-container-col--10`）内的 textarea
  - 5 个视频 = 5 个评分原因，6 个视频 = 6 个评分原因，与列数严格对应
- 顺带修复"必答未填"扫描：以前同样会把无关 textarea 误判为必填

---

## [1.9.74] - 2026-05-15

### 修复：定位 textarea 类未答题时滚动位置不准
- "定位未答题"跳到"整体评分原因"等 textarea 时，复用评分题定位的滚动锚点
  - textarea 的滚动锚点切换为它前面最近的评分组 → 滚动行为和评分题完全一致
  - 不再出现"红框被定位但要自己手动滚才看见"的情况
- 短路判定时也用锚点 rect，避免 textarea 自身高度大引起误判
...
### 美学专项（1-10 分）色阶
- 1-10 分按钮 / 胶囊改为**红→绿渐变**：1 分最红（最低），10 分最深绿（最高）
  - 视觉一眼分辨好坏：分越高颜色越偏绿、分越低越偏红
  - 工具栏面板"一键全选"按钮 + 列顶"本列全选" + 维度旁胶囊 全部同步染色
- 老模板（4 档评分）配色保持不变，零回归
...
- **自动聚焦顺序更符合作业流**（首次进入页面）：
  视频1·整体评分 → 整体评分原因（textarea）→ 视频1·维度1 → 维度2 → … → 视频2·整体评分 → …
  评分原因 textarea 为空也算"未答"，按 DOM 顺序自然衔接。
- **"维度打分后自动跳转"开关** 在美学专项模板下**默认关闭**（旧 4 档模板保持默认开启）
  - 两个模板各自记忆开关状态，互不影响。
- **"本列全选"批量打分后**：自动聚焦该列的"整体评分原因"输入框（让你立刻填理由）
  - 如果原因已填，则跳到下一道未答题（保持原行为）。
- **Tab / ↓ / 数字键打完分** 的自动跳转 也会经过 textarea：原因没填时焦点会停在 textarea。
...
### 改进
- **未答题红框收紧**：只框选"那一题"本身，不再把整个题目卡片/列表框成一大块
  - 评分组（`.cr-radio-group`）只框评分按钮区
  - 评分原因 textarea 只框输入框本身
- **文案**：视频区"自动循环"开关改名为"播放循环"，更直观

---

## [1.9.70] - 2026-05-15

### 新增：适配 美学专项（1-10 分制）模板
- **模板自动识别**：`QLBMode.template` 现在区分三种 → `label-aesthetic10` / `label-old4` / `qa-old`
  - 通过页面 `label[name="2"]` ~ `label[name="10"]` 判定为美学版
  - 切任务自动切换，无需手动配置
- **1-10 分制完整支持**：
  - 工具栏"一键全选"和列顶/维度胶囊改为按模板**动态生成**（旧 4 档保持单排，1-10 分两排 5+5）
  - 修复 v1.9.69 残留：之前新模板页面会显示无效的"0/0.5/1/none"按钮，现已根据模板自动重画
  - 进度统计兼容（已答 X / 总 Y）
- **快捷键映射**（按模板自动切换）：
  - 美学版：`1` ~ `9` → 1~9 分；`` ` `` / `~` / `0` → 10 分
  - 旧版：`1` → 0 分、`2` → 0.5 分、`3` → 1 分、`` ` `` / `~` / `4` → none

### 新增：评分原因 textarea 全面支持
- **必填判定**：美学版每题"整体评分原因"输入框算作必答题；填了任意内容就算已答
- **草稿自动保存 textarea 内容**：每打字触发去抖保存；每 50 字 toast 一次"💾 已自动保存草稿"
- **草稿恢复 textarea 内容**：一键恢复时连同评分原因一起回填
- **进度显示**：美学版工具栏进度区额外显示"原因 X/Y"（已填写的评分原因数量）

### 新增：textarea 快捷键
- 在 textarea 内按 **Cmd/Ctrl + Enter** → 跳到下一道未答题（比 Tab 更明确语义）

### 改进：草稿系统更可靠
- 草稿保存 `templateVersion` 字段，恢复时**校验模板版本**
  - 跨模板恢复（如：旧 4 档草稿 → 新 1-10 页面）会被拒绝，避免点错
- 维度名提取改为**优先读 group 内 `.cr-label > span`**（兼容新模板），旧 col--16 路径作 fallback
...
### 新增
- **答题本地草稿自动保存**（兜底 QLabel 平台偶发丢数据）：
  - 每次打分 / 修正分输入后自动把当前所有分值快照存到 localStorage（300ms 去抖）
  - 关标签 / 刷新 / 崩溃前最后兜底同步写一次（`beforeunload`）
  - 按任务 URL 区分存储，不会串扰
  - **完全本地，不上传任何数据；7 天后自动清理**
- **工具栏新增 📋 草稿按钮**：
  - 显示当前任务的草稿摘要（题数、时间、模式）
  - **♻️ 一键恢复到当前页**：按"维度名+视频列"业务锚点匹配，模拟 label 点击重放打分
    - 不会覆盖已有分值（只填空槽位）
    - 标注 + 质检模式都支持，质检模式额外恢复修正分输入
  - **📋 复制草稿 JSON**：导出到剪贴板，便于备份到备忘录 / 文档
  - **🗑 清除此草稿**：提交成功后手动清除
  - 列出其它任务的草稿（跨 URL）便于管理

### 使用建议
- 同事刷新页面发现数据丢了？→ 工具栏 📋 → 一键恢复
- 遇到视频加载失败：**别慌张刷新**，先点工具栏 📋 确认草稿已保存，再刷新；刷新后如果页面空了就点恢复
- 提交成功后记得手动"清除此草稿"（下一版本会做自动清除）
...
### 改进
- **质检模式聚焦框收紧**：
  - 之前：蓝色聚焦框覆盖**整行**（维度名+平均分 / 通过-不通过 / 修正分输入）三段都框住，视觉上很大很乱
  - 现在：蓝色聚焦框只覆盖"**维度名+平均分**"+"**是否通过 + 通过/不通过**"两段，不再框住宽宽的修正分输入区
  - 紧凑、视觉聚焦，与你打分的实际操作目标对齐
- 红色脉冲（定位未答题）保持覆盖整行不变 —— 那个场景下视觉提醒越强越好
...
### 改进
- **"维度打分后自动跳转"开关默认改为开启**：
  - 因为 v1.9.65 已经把"焦点移动"和"视口滚动"解耦（关闭开关时焦点不动也不滚视口），开启的负面影响大大降低
  - 默认开启更符合"打完一题继续打下一题"的连续工作流
  - 注意：**已经装过插件的老用户**升级后保持原值（关闭），新用户/重置偏好后默认开启
- **横向滚动抑制：聚焦切换时不再打乱用户已对齐的横向位置**：
  - 场景：用户手动横滑到"题目列和视频列正好对齐"的合适位置后，点 通过/不通过 触发自动聚焦时，旧版会把列再卡到正中央 → 用户的对齐被打乱
  - 新策略：检查焦点题对应的视频列**是否已经完全可见**——是 → 视为合理对齐，保持现状不动横滚；否 → 才主动对齐
  - 用户体验：自己手动滑到的位置被尊重，自动聚焦不再"咔哒一下"
